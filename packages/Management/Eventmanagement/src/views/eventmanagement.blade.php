            ...
            <div class="box">
                <div class="box-body">
                    <table id="data_list" class="table table-bordered">
                        ...
                        <tbody>
                            @foreach($eventmanagement as $key=>$item)
                                <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$item->title}}</td>
                                ...
                                <td>{{$item->remark}}</td>
                                    ...
                                    <td><a href="{{url('admin/eventmanagement/'.$item->id.'/edit')}}" style="border-radius:50%" class="btn btn-success"><i class="fa fa-edit"></i></a></td>

                                    <td>   <form method="post" action="{{url('admin/eventmanagement/'.$item->id)}}">                   
                                            @csrf
                                            @method('DELETE')
